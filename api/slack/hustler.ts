...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.body?.event?.type === 'app_mention') {
    ...
    await fetch('https://slack.com/api/chat.postMessage', {
      ...
      body: JSON.stringify({ channel: slackEvent.channel, text: `_Crunching the data and structuring the plays. Give me a minute to strategize, Boss._`, thread_ts: slackEvent.ts })
    });
...
If you need The Closer to draft a cold email based on your strategy, ping him by appending exactly <@${process.env.SLACK_CLOSER_USER_ID || 'U0B7X27TE1F'}> to the very end of your message.
