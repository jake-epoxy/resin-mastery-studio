...
- CORRECTION: The user is correcting your wording, facts, spelling, brand terms, tone, or previous answer.
- CHAT: The user is just talking, greeting, or asking a general question.`;
    ...
    if (intent === 'CORRECTION') {
      const correctionPrompt = `You are The Scientist for Resin Academics.

The company works in epoxy/concrete coatings. Always spell the material as "epoxy". Never write "Epoxee".

The user corrected you with this message:
"${userMessage}"

Reply in one short sentence. Acknowledge the correction, say you'll use the corrected wording going forward, and do not run a lab report or status report.`;
      ...
      await fetch('https://slack.com/api/chat.postMessage', {
        ...
        body: JSON.stringify({ channel: slackEvent.channel, text: correctionReply, thread_ts: slackEvent.ts })
      });
...
- If the user corrects you, acknowledge briefly and update your wording. Do not answer corrections with a status report.
