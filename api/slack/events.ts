...
Based on the user's message, reply with ONLY the exact capitalized name of the agent who should handle it (e.g., SCOUT, CLOSER, HUSTLER, SCIENTIST, OPERATOR). Do not include any other text.
      ...
      let nextAgent: string | null = response.choices[0].message.content?.trim().toUpperCase() || 'OPERATOR';
      let currentInput = userMessage;
      let depth = 0;

      while (nextAgent && depth < 3) {
        let finalReply = "";
        let agentEmoji = "⚙️";
        ...
        if (nextAgent.includes("SCIENTIST")) {
...
If you need the Closer to draft an email or the Hustler to structure a business deal based on your ideas, append EXACTLY "[CHAIN:@CLOSER]" or "[CHAIN:@HUSTLER]" to the very end of your response.
...
Respond in pure JSON format with exactly these fields:
{
  ...
  "chain_to": "If you need another agent to do something next, put their name here like '[CHAIN:@HUSTLER]', otherwise leave blank."
}`;
...
If you need the Closer to draft a cold email based on your strategy, append EXACTLY "[CHAIN:@CLOSER]" to the very end of your response.
...
The user asked: ${currentInput}`;
        ...
        // Parse Chain Tag
        let newNextAgent = null;
        const chainMatch = finalReply.match(/\[CHAIN:@([A-Z]+)\]/);
        
        if (chainMatch) {
          newNextAgent = chainMatch[1];
        ...
        if (newNextAgent) {
          nextAgent = newNextAgent;
          currentInput = `A previous agent compiled this output:\n${finalReply}\n\nPlease take the next step and execute your role based on this.`;
        } else {
          nextAgent = null; // Break loop
        }
